...
const num = [10,'10',6,101,10_000];

// ~ 보다 큼 또는 작음 → boolean
let lessThen = num[0] < num[3];
let greaterThen = num[0] > num[2];
console.log(lessThen, greaterThen);


// ~ 보다 크거나 작음 또는 같음 → boolean
let lessThenAndEqual = num[0] <= num[1];
let greaterThenAndEqual = num[0] >= num[1];
console.log(lessThenAndEqual, greaterThenAndEqual);
 

// 동등(등급이나 정도가 같다) → boolean
let equal = num[0] == num[1];
console.log(equal);


// 다름(같지 않음) → boolean
let unequal = num[0] != num[1];
console.log(unequal);


// 동일(똑같다) → boolean
let same = num[0] === num[1];
console.log(same);
